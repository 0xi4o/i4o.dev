...
date_published: 2021-08-27T03:16:00.000Z
date_updated: 2022-01-06T05:58:06.000Z
...
_1. _[ESLint](*https://eslint.org/*): Identifies issues in the javascript and typescript files I write, enforces rules for syntax and patterns.
_2. _[Prettier](*https://prettier.io/*): Enforces a global code-style no matter how the code is formatted while writing.
_3. _`husky` + `lint-staged`: Runs eslint and prettier every time I try to commit to source control. This makes sure that the code I push to Github is properly formatted and doesn't have any syntactical problems or anti-patterns. I don't write tests (*\_*gasps*\_*) but if I did, this is where we can run our tests as well.
_4. _[SWR](*https://swr.vercel.app/*): React hooks for data fetching that is super fun to use and [supercharges how we make API calls](*https://ilango.hashnode.dev/supercharging-data-fetching-in-react-with-swr*) in our applications.
_5. _[Formik](*https://formik.org*): Formik makes working with forms incredibly easy by keeping track of changes in values, errors, visited fields. This way, we spend a lot less time handling these things.
_6. _[Yup](*https://github.com/jquense/yup*): Formik does not do data validation in forms. We're free to write our own, but in the interest of moving fast, I use Yup (recommended by Formik). It's fast and quite easy to use.
